...
class Whirl_Productrecommendation_Model_Views extends Mage_Core_Model_Abstract
{
    ...
    public function getMapByGroup($skuCode, $associateCode) {
    	...
    	->where('also_viewed_sku_code = :associated_sku_code');
    	 
    	$bind = array(':sku_code' => $skuCode, ':associated_sku_code' => $associateCode);
    	$viewMapId = $helper->getReadAdapter()->fetchRow($select, $bind);
    	if(!empty($viewMapId)) {
    		return $viewMapId;
    	}
